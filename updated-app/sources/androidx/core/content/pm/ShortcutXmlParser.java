...
public class ShortcutXmlParser {
        ...
        try {
            for (ResolveInfo info : resolveInfos) {
                ...
                if (metaData != null && metaData.containsKey(META_DATA_APP_SHORTCUTS)) {
                    parser = getXmlResourceParser(context, activityInfo);
                    result.addAll(parseShortcutIds((XmlPullParser) parser));
                    if (parser != null) {
                        parser.close();
                    }
                }
            ...
            ShortcutXmlParser$$ExternalSyntheticBackport0.m(th, th);
        }
        return result;
        throw th;
    }
    ...
    public static List<String> parseShortcutIds(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (true) {
            ...
            int type = next;
            if (next == 1 || (type == 3 && parser.getDepth() <= 0)) {
                return result;
            }
