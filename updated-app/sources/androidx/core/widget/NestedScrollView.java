...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent3, NestedScrollingChild3, ScrollingView {
            ...
            case 2:
                ...
                if (activePointerIndex != -1) {
                    ...
                    if (this.mIsBeingDragged == 0) {
                        ...
                        dispatchNestedScroll(0, scrolledDeltaY, 0, deltaY2 - scrolledDeltaY, this.mScrollOffset, 0, this.mScrollConsumed);
    ...
    public int consumeFlingInVerticalStretch(int unconsumedY) {
        ...
        if (unconsumedY > 0 && EdgeEffectCompat.getDistance(this.mEdgeGlowTop) != 0.0f) {
            int consumed = Math.round((((float) (-height)) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (((float) (-unconsumedY)) * FLING_DESTRETCH_FACTOR) / ((float) height), MAX_SCROLL_FACTOR));
            ...
            int consumed2 = Math.round((((float) height) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, (((float) unconsumedY) * FLING_DESTRETCH_FACTOR) / ((float) height), MAX_SCROLL_FACTOR));
    ...
    private float getSplineFlingDistance(int velocity) {
        ...
        double d3 = (double) DECELERATION_RATE;
        Double.isNaN(d3);
        ...
        if (event.getAction() == 8 && !this.mIsBeingDragged) {
            ...
            if (vscroll != 0.0f) {
                ...
                if (newScrollY < 0) {
                    ...
                    if (canOverScroll) {
                        EdgeEffectCompat.onPullDistance(this.mEdgeGlowTop, (-((float) newScrollY)) / ((float) getHeight()), MAX_SCROLL_FACTOR);
                        this.mEdgeGlowTop.onRelease();
                        invalidate();
                        absorbed = true;
                    }
                    ...
                    if (canOverScroll) {
                        EdgeEffectCompat.onPullDistance(this.mEdgeGlowBottom, ((float) (newScrollY - range)) / ((float) getHeight()), MAX_SCROLL_FACTOR);
                        this.mEdgeGlowBottom.onRelease();
                        invalidate();
                        absorbed = true;
                    }
