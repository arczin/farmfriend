...
public abstract class ButtonBase extends AndroidViewComponent implements View.OnClickListener, View.OnFocusChangeListener, View.OnLongClickListener, View.OnTouchListener, AccessibleComponent {
    ...
    private void updateAppearance() {
        if (this.backgroundImageDrawable == null) {
            ...
            int shapeHeight = Math.round(((float) this.backgroundImageDrawable.getIntrinsicHeight()) * displayDensity);
            int shapeWidth = Math.round(((float) this.backgroundImageDrawable.getIntrinsicWidth()) * displayDensity);
            Bitmap result = Bitmap.createBitmap(shapeWidth, shapeHeight, Bitmap.Config.ARGB_8888);
            Canvas canvas = new Canvas(result);
            switch (this.shape) {
                case 1:
                    canvas.drawRoundRect(new RectF(0.0f, 0.0f, (float) shapeWidth, (float) shapeHeight), 100.0f, 100.0f, paint);
                    break;
                case 2:
                    canvas.drawRect(new RectF(0.0f, 0.0f, (float) shapeWidth, (float) shapeHeight), paint);
                    break;
                case 3:
                    canvas.drawOval(new RectF(0.0f, 0.0f, (float) shapeWidth, (float) shapeHeight), paint);
                    break;
                default:
    ...
    private void setShape() {
        ...
        if (Build.VERSION.SDK_INT >= 21) {
            if (this.backgroundColor == 16777215) {
                this.view.setOutlineProvider((ViewOutlineProvider) null);
                this.view.setStateListAnimator((StateListAnimator) null);
            } else {
