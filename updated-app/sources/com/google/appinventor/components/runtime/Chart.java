...
@SimpleObject
@DesignerComponent(category = ComponentCategory.CHARTS, description = "A component that allows visualizing data", iconName = "images/chart.png", version = 4)
...
public class Chart extends AndroidViewComponent implements ComponentContainer, OnInitializeListener {
    ...
    private void reinitializeChart() {
        ...
        Labels(this.labels);
        AxesTextColor(this.axesTextColor);
    }
    ...
    public void AxesTextColor(int argb) {
        if (argb == 0) {
            argb = $form().isDarkTheme() ? -1 : -16777216;
        }
        this.axesTextColor = argb;
        if (this.chartView instanceof PointChartView) {
            ((PointChartView) this.chartView).setAxesTextColor(argb);
        }
    }
    ...
    @DesignerProperty(editorType = "chart_value_type")
    @SimpleProperty(category = PropertyCategory.APPEARANCE, userVisible = false)
    public void ValueFormat(int valueType2) {
        this.valueType = valueType2;
        if (this.chartView instanceof AxisChartView) {
            ((AxisChartView) this.chartView).setValueType(valueType2);
        }
        for (ChartComponent dataComponent : this.dataComponents) {
            if (dataComponent.getDataModel() != null) {
                dataComponent.getDataModel().setChartValueType(valueType2);
            }
        }
    }
    ...
    public void ExtendDomainToInclude(double x) {
        if (this.chartView instanceof AxisChartView) {
            double[] bounds = ((AxisChartView) this.chartView).getXBounds();
            if (x < bounds[0]) {
    ...
    public void ExtendRangeToInclude(double y) {
        if (this.chartView instanceof AxisChartView) {
            double[] bounds = ((AxisChartView) this.chartView).getYBounds();
            if (y < bounds[0]) {
    ...
    public void ResetAxes() {
        if (this.chartView instanceof AxisChartView) {
            ((AxisChartView) this.chartView).resetAxes();
            refresh();
        }
    }
