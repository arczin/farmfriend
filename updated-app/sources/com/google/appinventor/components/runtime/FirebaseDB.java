...
public class FirebaseDB extends AndroidNonvisibleComponent implements Component {
    ...
    private void connectFirebase() {
        ...
        if (this.useDefault) {
            ...
            String str2 = this.developerBucket;
            this.myFirebase = new Firebase(str + "developers/" + str2 + this.projectBucket);
        } else {
            String str3 = this.firebaseURL;
            this.myFirebase = new Firebase(str3 + this.projectBucket);
        }
                ...
                try {
                    if (value instanceof String) {
                        ...
                        if (!(value2 instanceof List)) {
                            ...
                            return Transaction.abort();
                        } else if (((List) value2).isEmpty()) {
                            returnVal.err = "The list was empty";
                            return Transaction.abort();
                        } else {
                            returnVal.retval = ((List) value2).remove(0);
                            try {
        ...
        this.myFirebase.child("").addListenerForSingleValueEvent(new ValueEventListener() {
            public void onDataChange(DataSnapshot data) {
                ...
                if (value instanceof HashMap) {
                    final List listValue = (List) new ArrayList(((HashMap) value).keySet());
                    FirebaseDB.this.androidUIHandler.post(new Runnable() {
                        public void run() {
