...
public class Map extends MapFeatureContainerBase implements MapFactory.MapEventListener {
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE, description = "The type of tile layer to use as the base of the map. Valid values are: 1 (Roads), 2 (Aerial), 3 (Terrain), 4 (Custom)")
    public int MapType() {
    ...
    public void BoundingBox(YailList boundingbox) {
        YailList yailList = boundingbox;
        double latNorth = GeometryUtil.coerceToDouble(((YailList) yailList.get(1)).get(1));
        double longWest = GeometryUtil.coerceToDouble(((YailList) yailList.get(1)).get(2));
        double latSouth = GeometryUtil.coerceToDouble(((YailList) yailList.get(2)).get(1));
        double longEast = GeometryUtil.coerceToDouble(((YailList) yailList.get(2)).get(2));
        BoundingBox boundingBox = r3;
        double d = latSouth;
        BoundingBox boundingBox2 = new BoundingBox(latNorth, longEast, latSouth, longWest);
        this.mapController.setBoundingBox(boundingBox);
    }
    ...
    public YailList BoundingBox() {
        ...
        return YailList.makeList((Object[]) new YailList[]{YailList.makeList((Object[]) new Double[]{Double.valueOf(bbox.getLatNorth()), Double.valueOf(bbox.getLonWest())}), YailList.makeList((Object[]) new Double[]{Double.valueOf(bbox.getLatSouth()), Double.valueOf(bbox.getLonEast())})});
    }
    ...
    public void Save(String path) {
        ...
        if (path.startsWith("/") || path.startsWith("file:/")) {
            if (path.startsWith("file:")) {
                target = new File(URI.create(path));
            } else {
        ...
        final List<MapFactory.MapFeature> list = featuresToSave;
        new FileWriteOperation(this, $form(), this, "Save", path, $form().DefaultFileScope(), false, true) {
            ...
            public void processFile(ScopedFile file) {
                this.this$0.doSave(list, new File(URI.create(FileUtil.resolveFileName(this.form, file))));
            }
        }.run();
    }

    /* access modifiers changed from: private */
    public void doSave(List<MapFactory.MapFeature> featuresToSave, File target) {
        try {
