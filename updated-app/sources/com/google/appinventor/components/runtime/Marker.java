...
@SimpleObject
@DesignerComponent(category = ComponentCategory.MAPS, description = "<p>An icon positioned at a point to indicate information on a map. Markers can be used to provide an info window, custom fill and stroke colors, and custom images to convey information to the user.</p>", iconName = "images/marker.png", version = 4)
...
public class Marker extends MapFeatureBaseWithFill implements MapFactory.MapMarker {
    ...
    public int Width() {
        ...
        Double.isNaN(d);
        double width2 = (double) this.map.getView().getWidth();
        Double.isNaN(width2);
        return (int) (((d - 0.00408935546875d) / 100.0d) * width2);
    }
    ...
    public int Height() {
        ...
        Double.isNaN(d);
        double height2 = (double) this.map.getView().getHeight();
        Double.isNaN(height2);
        return (int) (((d - 0.00408935546875d) / 100.0d) * height2);
    }
