...
public class NxtDrive extends LegoMindstormsNxtBase {
    ...
    public void beforeDisconnect(BluetoothConnectionBase bluetoothConnection) {
        if (this.stopBeforeDisconnect) {
            for (NxtMotorPort outputState : this.driveMotorPorts) {
    ...
    private void move(String functionName, int power, long tachoLimit) {
        if (checkBluetooth(functionName)) {
            for (NxtMotorPort outputState : this.driveMotorPorts) {
                setOutputState(functionName, outputState, power, NxtMotorMode.On, NxtRegulationMode.Speed, 0, NxtRunState.Running, tachoLimit);
            }
        }
    }
    ...
    private void turnIndefinitely(String functionName, int power, int forwardMotorIndex, int reverseMotorIndex) {
        if (checkBluetooth(functionName)) {
            ...
            setOutputState(str, this.driveMotorPorts.get(forwardMotorIndex), power, NxtMotorMode.On, NxtRegulationMode.Speed, 0, NxtRunState.Running, 0);
            setOutputState(str, this.driveMotorPorts.get(reverseMotorIndex), -power, NxtMotorMode.On, NxtRegulationMode.Speed, 0, NxtRunState.Running, 0);
        }
    }
