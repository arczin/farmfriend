...
public class PieChartView extends ChartView<PieEntry, IPieDataSet, PieData, PieChart, PieChartView> {
    ...
    public void resizePieRings() {
        int lastWidth = 0;
        int lastHeight = 0;
        float newHoleRadius = 100.0f - ((100.0f - ((float) this.pieHoleRadius)) * (((((float) this.pieHoleRadius) / 100.0f) + 0.75f) / ((float) this.pieCharts.size())));
        for (int i = 0; i < this.pieCharts.size(); i++) {
            PieChart pieChart = this.pieCharts.get(i);
            boolean lastChart = true;
            if (i != this.pieCharts.size() - 1) {
                lastChart = false;
            }
    ...
    private void changePieChartRadius(PieChart pieChart, float newHoleRadius, boolean lastChart) {
        if (!lastChart) {
            ...
            float setRadius = ((float) this.pieHoleRadius) * (((newHoleRadius - ((float) this.pieHoleRadius)) / 100.0f) + 1.0f);
