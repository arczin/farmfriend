...
public class Slider extends AndroidViewComponent implements SeekBar.OnSeekBarChangeListener {
    ...
    private static final int initialThumbColor = -12303292;
    private static final String initialThumbColorString = "&HFF444444";
    ...
    public Slider(ComponentContainer container) {
        ...
        this.seekbar.setMax(this.numberOfSteps);
        setSeekbarPosition();
    }
    ...
    private void setSliderColors() {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            this.seekbar.setThumbTintList(ColorStateList.valueOf(this.thumbColor));
    ...
    private void setSeekbarPosition() {
        float seekbarPosition = ((this.thumbPosition - this.minValue) / (this.maxValue - this.minValue)) * ((float) this.numberOfSteps);
        if (Build.VERSION.SDK_INT >= 24) {
            this.seekbar.setProgress((int) seekbarPosition, this.notice);
        } else {
    ...
    @SimpleProperty(description = "Sets the minimum value of slider. If the new minimum is greater than the current maximum, then minimum and maximum will both be set to this value.", userVisible = true)
    public void MinValue(float value) {
        this.minValue = value;
        this.maxValue = Math.max(value, this.maxValue);
        this.thumbPosition = (((this.maxValue - this.minValue) * ((float) this.seekbar.getProgress())) / ((float) this.numberOfSteps)) + this.minValue;
    }
    ...
    @SimpleProperty(description = "Sets the maximum value of slider. If the new maximum is less than the current minimum, then minimum and maximum will both be set to this value. ", userVisible = true)
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE, description = "Number of steps on the slider scale. Combined withMinValue and MaxValue, it allows you to get the slider precision that you want, e.g. MinValue = 0,MaxValue = 150, NumberOfSteps = 1000, the slider will change position every 0.15.", userVisible = true)
    ...
    public void NumberOfSteps(int value) {
        ...
        float oldPosition = this.thumbPosition;
        this.notice = false;
        this.seekbar.setMax(value);
        this.thumbPosition = oldPosition;
        setSeekbarPosition();
        this.notice = true;
    }
    ...
    @SimpleProperty(category = PropertyCategory.APPEARANCE, description = "The slider thumb color")
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        if (this.notice) {
            this.thumbPosition = (((this.maxValue - this.minValue) * ((float) progress)) / ((float) this.numberOfSteps)) + this.minValue;
