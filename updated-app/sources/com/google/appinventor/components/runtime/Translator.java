...
public final class Translator extends AndroidNonvisibleComponent {
    ...
    public void performRequest(String languageToTranslateTo, String textToTranslate) {
        Exception e;
        String returnText;
        String srcLanguage;
        String returnText2;
        ...
        try {
            ...
            if (languages.length == 2) {
                String srcLanguage2 = languages[0];
                targetLanguage = languages[1];
                srcLanguage = srcLanguage2;
            } else {
                srcLanguage = "auto";
            }
            try {
                TranslatorToken.request request = TranslatorToken.request.newBuilder().setToken(TranslatorToken.token.parseFrom(Base58Util.decode(this.apiKey))).setTotranslate(textToTranslate).setSourcelanguage(srcLanguage).setTargetlanguage(targetLanguage).build();
                connection = (HttpsURLConnection) new URL(TRANSLATOR_SERVICE_URL).openConnection();
                if (connection != null) {
                    ...
                    if (responseCode == 200) {
                        returnText2 = response.getTranslated();
                    } else {
                        returnText2 = getResponseContent(connection, false);
                    }
                    String str = returnText2;
                    GotTranslation(new StringBuilder().append(responseCode).toString(), returnText2);
    ...
    @SimpleProperty(category = PropertyCategory.ADVANCED, description = "The API Key to use. MIT App Inventor will automatically fill this value in. You should not need to change it.", userVisible = true)
