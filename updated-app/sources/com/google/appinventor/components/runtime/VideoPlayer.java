...
public final class VideoPlayer extends AndroidViewComponent implements OnDestroyListener, Deleteable, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener, MediaPlayer.OnPreparedListener {
    ...
    public void Source(@Asset String path) {
        String str = "";
        final String tempPath = path == null ? str : path;
        if (!TiramisuUtil.requestVideoPermissions(this.container.$form(), path, new PermissionResultHandler() {
    ...
    public void Volume(int vol) {
        ...
        if (this.mPlayer != null) {
            this.mPlayer.setVolume(((float) vol2) / 100.0f, ((float) vol2) / 100.0f);
        }
    }
    ...
    public boolean onError(MediaPlayer m, int what, int extra) {
        ...
        this.delayedStart = false;
        this.mediaReady = false;
        String hexString = Integer.toHexString(what);
        Log.e("VideoPlayer", "onError: what is " + what + " 0x" + hexString + ", extra is " + extra + " 0x" + Integer.toHexString(extra));
        this.container.$form().dispatchErrorOccurredEvent(this, "Source", ErrorMessages.ERROR_UNABLE_TO_LOAD_MEDIA, this.sourcePath);
        return true;
    }
    ...
    class ResizableVideoView extends VideoView {
        ...
        public void onMeasure(int specwidth, int specheight, int trycount) {
            ...
            Log.i("VideoPlayer..onMeasure", "AI setting dimensions as:" + i4 + ":" + this.forcedHeight);
            int size = View.MeasureSpec.getSize(specwidth);
            Log.i("VideoPlayer..onMeasure", "Dimenions from super>>" + size + ":" + View.MeasureSpec.getSize(specheight));
            int width = 176;
            int height = 144;
                ...
                case -1:
                    if (this.mFoundMediaPlayer.booleanValue()) {
                        try {
                            ...
                            NullPointerException nullPointerException = nullVideoPlayer2;
