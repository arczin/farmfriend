...
public final class GeometryUtil {
    ...
    public static Coordinate[] pointsToCoordinates(List<GeoPoint> points) {
        ...
        Coordinate[] coordinates = new Coordinate[(points.size() + (closed ^ true ? 1 : 0))];
        int i = 0;
    ...
    public static double distanceBetween(IGeoPoint a, IGeoPoint b) {
        ...
        double d2 = lng1;
        return 6378137.0d * Math.atan2(Math.sqrt(cordlen), Math.sqrt(1.0d - cordlen)) * 2.0d;
    }
    ...
    public static double distanceBetweenEdges(MapFactory.MapMarker marker, MapFactory.MapCircle circle) {
        double distanceTo = (double) marker.getCentroid().distanceTo(circle.getCentroid());
