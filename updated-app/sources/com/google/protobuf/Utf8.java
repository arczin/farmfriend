        ...
        public final int partialIsValidUtf8Default(int state, ByteBuffer buffer, int index, int limit) {
            if (state != 0) {
                ...
                if (byte1 < -32) {
                    ...
                    if (byte22 <= -65 && (((byte1 << 28) + (byte22 + Ev3Constants.Opcode.JR_NEQ8)) >> 30) == 0 && byte3 <= -65) {
                        int index7 = index + 1;
                        if (buffer.get(index) > -65) {
                            int i3 = index7;
                        } else {
        ...
        public final void encodeUtf8Default(CharSequence in, ByteBuffer out) {
            ...
            while (inIx < inLength) {
                ...
                if (c2 < 128) {
                    ...
                    outIx = outIx2 + 1;
                    try {
                        out.put(outIx2, (byte) ((c2 >>> 6) | FullScreenVideoUtil.FULLSCREEN_VIDEO_ACTION_PAUSE));
        ...
        public int encodeUtf8(CharSequence in, byte[] out, int offset, int length) {
            ...
            while (inIx < inLimit) {
                ...
                if (c4 < c && outIx < outLimit) {
                    ...
                    outIx = outIx4 + j;
                    UnsafeUtil.putByte(bArr, outIx4, (byte) ((c4 & '?') | 128));
                    outIx2 = outLimit;
                    c = 128;
                    ...
                    UnsafeUtil.putByte(bArr, outIx5, (byte) (((c4 >>> 6) & 63) | 128));
                    UnsafeUtil.putByte(bArr, outIx6, (byte) ((c4 & '?') | 128));
                    outIx2 = outLimit;
                    outIx = outIx6 + 1;
                    c = 128;
                    ...
                    if (55296 > c4 || c4 > 57343 || (inIx + 1 != inLimit && Character.isSurrogatePair(c4, charSequence.charAt(inIx + 1)))) {
                ...
                while (inIx < inLimit) {
                    ...
                    if (c4 < c && outIx < outLimit) {
                        ...
                        if ((c4 < 55296 || 57343 < c4) && outIx <= outLimit - 3) {
                            ...
                            String str3 = str2;
                            if (55296 > c4 || c4 > 57343 || (inIx + 1 != inLimit && Character.isSurrogatePair(c4, charSequence.charAt(inIx + 1)))) {
                                throw new ArrayIndexOutOfBoundsException("Failed writing " + c4 + str3 + outIx);
                            }
                            throw new UnpairedSurrogateException(inIx, inLimit);
                        }
                    } else {
