...
public abstract class RealNum extends Complex implements Comparable {
    ...
    public static RealNum add(RealNum x, RealNum y, int k) {
        RealNum realNum = (RealNum) x.add(y, k);
        RealNum realNum2 = realNum;
        return realNum;
    }

    public static RealNum times(RealNum x, RealNum y) {
        RealNum realNum = (RealNum) x.mul(y);
        RealNum realNum2 = realNum;
        return realNum;
    }

    public static RealNum divide(RealNum x, RealNum y) {
        RealNum realNum = (RealNum) x.div(y);
        RealNum realNum2 = realNum;
        return realNum;
    }
    ...
    public static int toStringScientific(String dstr, StringBuffer sbuf) {
        ...
        if (dstr.charAt(pos2) == '0') {
            ...
            while (true) {
                ...
                pos = pos2 + 1;
                ch2 = dstr.charAt(pos2);
                if (ch2 < '0' || ch2 > '9' || (ch2 == '0' && pos != len)) {
                    pos2 = pos;
                }
            }
            sbuf.append(ch2);
            ...
            int exp2 = ((len - i) - len) + dstr.indexOf(46);
